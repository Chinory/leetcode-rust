...
impl Solution {
  pub fn my_atoi(s: String) -> i32 {
    ...
    /* x: initial result, n: negaive */
    let (x, n) = match it.next() {
      Some('+') => (0, false), Some('-') => (0, true),
      Some(c @'0'..='9') => (ctoi(c), false), _ => return 0
    };
    let mut it = it.take_while(|&c|'0'<=c && c<='9');
    if n { it.try_fold(x, |i,c|
      i.checked_mul(10)?.checked_sub(ctoi(c))).unwrap_or(i32::MIN) }
    else { it.try_fold(x, |i,c|
      i.checked_mul(10)?.checked_add(ctoi(c))).unwrap_or(i32::MAX) }
  }
}
